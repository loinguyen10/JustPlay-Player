...
class ChannelData {
  ...
  factory ChannelData.fromMap(Map<String, dynamic> map) {
    ...
    String? subscribers = headers?.get('c4TabbedHeaderRenderer')?.get('subscriberCountText')?['simpleText'];
    ...
    String? banner = headers?.get('c4TabbedHeaderRenderer')?.get('banner')?.getList('thumbnails')?.first['url'];
    var contents = map
        .get('contents')
    ...
    return ChannelData(
        videosList: videoList, channel: ChannelPage(subscribers: (subscribers != null) ? subscribers : " ", avatar: avatar, banner: banner));
  }
}
